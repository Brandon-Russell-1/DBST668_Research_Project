...
SELECT /*fixed*/ a.Instr_LName, a.Instr_FName, b.Class_Room, c.Sched_Day, c.Sched_Time
  FROM (Instr_List a INNER JOIN Instr_Classes b ON a.Instr_Num=b.Instr_Num)
...
SELECT /*fixed*/ Section_Name, COUNT(Instr_Classes.Instr_Num) AS Number_Of_Classes 
  FROM (Section_Info INNER JOIN Instr_List ON
    Section_Info.Section_Num=Instr_List.Section_Num) INNER JOIN Instr_Classes ON
      Instr_List.Instr_Num=Instr_Classes.Instr_Num
        GROUP BY Section_Name
          HAVING COUNT(Instr_Classes.Instr_Num) = 2;
  ...
  FROM Instr_List
    WHERE EXISTS
      (SELECT * FROM Instr_Classes INNER JOIN Course_List
        ON Instr_Classes.Course_Num=Course_List.Course_Num
        WHERE Instr_List.Instr_Num=Instr_Classes.Instr_Num
...
SELECT /*fixed*/ a.Instr_LName, a.Instr_FName, b.Class_Room, c.Course_Name
  FROM (Instr_List a LEFT JOIN Instr_Classes b ON a.Instr_Num=b.Instr_Num)
